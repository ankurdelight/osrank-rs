    ...
    R: Clone + Zero,
{
    fn dynamic_weight(
        ...
        let mut weight: Self::Weight = (hyperparams.get_param(e_type)).clone();
        ...
        match e_type {
            types::EdgeType::Contrib => {
                ...
                let total_project_contrib = source_node.data().total_contributions;
                let user_contribs = edge.data().contributions;

                match (user_contribs, total_project_contrib) {
                    (None, None) => panic!(
                        r###"Graph construction incorrect: found a contrib node
                        without contributions and an edge without contributions."###
                    ),
                    (Some(_), None) => panic!(
                        r###"Graph construction incorrect: found a contrib node
                        with contributions, but an edge without contributions."###
                    ),
                    (None, Some(_)) => panic!(
                        r###"Graph construction incorrect: found a contrib node
                        without contributions, but an edge with contributions."###
                    ),
                    (Some(uc), Some(total)) => weight = weight * Weight::new(uc, total).into(),
                }
            }
            types::EdgeType::ContribStar => {
                ...
                let total_account_contrib = source_node.data().total_contributions;
                let user_contribs = edge.data().contributions;

                match (user_contribs, total_account_contrib) {
                    (None, None) => panic!(
                        r###"Graph construction incorrect: found a contrib* node
                        without contributions and an edge without contributions."###
                    ),
...
where
    W: Zero,
{
    pub fn new_account(id: Id, total_contributions: u32) -> Self {
        Artifact {
            id,
    ...
    fn network_fixture() -> Network<Weight, Osrank> {
        ...
        for edge in &edges {
            network.add_edge(
                ...
                &edge.1.to_string(),
                types::EdgeData {
                    edge_type: types::EdgeType::Depend, //FIXME(adn) provisional
                    weight: edge.2,
                    contributions: None, //FIXME(adn) provisional
                },
            )
        }
